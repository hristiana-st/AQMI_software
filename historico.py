...
def app():
    ...
    filter_cols = [
        ...
        "Fibers", "Microcalcifications", "Masses",
    ...
    def plot_ts(col, baseline=None, limit=None):
        fig = px.scatter(
            df_filtered,
            x="Evaluation date",
            y=col,
            hover_data={c: True for c in hover_cols if c in df_filtered.columns},
            title=col
        )
    ...
    if "Fibers" in df_filtered.columns:
        with col1:
            plot_ts("Fibers", baseline=4)

    if "Microcalcifications" in df_filtered.columns:
        with col2:
            plot_ts("Microcalcifications", baseline=3)

    if "Masses" in df_filtered.columns:
        with col1:
            plot_ts("Masses", baseline=3)
    ...
    for metric in metrics:
        for x in x_vars:
            if metric not in df_filtered.columns or x not in df_filtered.columns:
                continue
