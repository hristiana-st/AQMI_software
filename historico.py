...
def app():
    ...
    # always rename columns to English
    ...
    # Full Dataset
    # -------------------
    full_dataset_title = t["full_dataset"]
    st.markdown(f"### {full_dataset_title}")
    ...
    filter_cols = [
        ...
        "Detector distance", "FOV dimension", "Filter type", "Focal spot", "Dose (mGy)"
    ]
    ...
    if "Evaluation date" in df_filtered.columns:
        ...
        unique_dates = df_filtered["Evaluation date"].sort_values().unique()

        with col1:
            data_inicio = st.selectbox(t["start_date"], unique_dates)
        with col2:
            data_fim = st.selectbox(t["end_date"], unique_dates)
    ...
    # Time Series Plots
    # -------------------
    time_series_title = t["time_series_plots"]

    def plot_time_series(column, y_limit=None, base_line=None):
        fig, ax = plt.subplots()
        ax.scatter(df_filtered["Evaluation date"], df_filtered[column])
        if base_line is not None:
            ax.plot(df_filtered["Evaluation date"], [base_line]*len(df_filtered), 'r--')
        ax.grid()
        fig.autofmt_xdate()
        if y_limit:
            ax.set_ylim(y_limit)
    ...
    with col1:
        if "Fibers" in df_filtered.columns:
            st.write(t.get("fibers", "Fibers"))
            plot_time_series("Fibers", y_limit=(0, 6.5), base_line=4)

    with col2:
        if "Microcalcifications" in df_filtered.columns:
            st.write(t.get("specks", "Microcalcifications"))
            plot_time_series("Microcalcifications", y_limit=(0, 5.5), base_line=3)

    with col1:
        if "Masses" in df_filtered.columns:
            st.write(t.get("masses", "Masses"))
            plot_time_series("Masses", y_limit=(0, 5.5), base_line=3)
    ...
    # -------------------
    # Scatter plots colored by Anode / Filter
    # -------------------

    # Create column for combination
    if "Anode material" in df_filtered.columns and "Filter material" in df_filtered.columns:
        df_filtered["Anode / Filter"] = df_filtered["Anode material"] + " / " + df_filtered["Filter material"]

    metrics = ["CNR", "SNR"]
    x_axes = ["Dose (mGy)", "kVp", "mAs"]
    ...
    for metric in metrics:
        ...
        for x in x_axes:
            if x in df_filtered.columns:
                df_plot = df_filtered[[x, metric, "Anode / Filter"]].drop_duplicates()
                fig = px.scatter(
                    df_plot,
                    x=x,
                    y=metric,
                    color="Anode / Filter",
                    title=f"{metric} vs {x}",
                    labels={x: x, metric: metric, "Anode / Filter": "Anode / Filter"}
                )
